...
export class ComplaintNotifierService {
  ...
  async notifyCommittee(complaint: Complaint): Promise<void> {
    const members = await this.usersService.getCommitteeMembers();
    if (members.length === 0) {
      this.logger.warn(`No committee members found for complaint ${complaint.id}`);
      return;
    }
    await this.sendNotificationToRecipients({
      ...
      messagePrefix: 'A new complaint has been submitted and requires your review.',
    });
  }
